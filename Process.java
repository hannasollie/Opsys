...
public class Process implements Constants
{
	...
	public void enteredCpu(long clock){ //legger til prosessen til CPU
		nofTimesInReadyQueue++;
		timeSpentInReadyQueue += clock - timeOfLastEvent;
		timeOfLastEvent = clock;

	}
	public void leftCpu(long clock) { //fjerner prosessen fra CPU og oppdaterer statistikk
	...
	public long timeToNextIO() {  //kalkulerer tiden til neste IO-operasjon
		if(timeToNextIoOperation == 0) {
			timeToNextIoOperation = (long) (Math.random()* avgIoInterval*2); //hvorfor gange med 2?
		}
		return timeToNextIoOperation;
	}

	public long requiredCpuTime() { //tiden prosessen trenger på CPU for å gjøre seg ferdig
